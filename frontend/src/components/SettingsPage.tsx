    ...
    <div className={`mt-2 p-3 rounded-md ${colors[strength.level]}`}>
      ...
      {!strength.isValid && (
        <p className="text-xs font-semibold mt-1">
          {t("settings.encryption_password_weak")}
        </p>
      )}
    </div>
  );
...
export function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!strength.isValid) {
      setMessage(
        `${t("settings.error_password_weak")}: ${strength.feedback[0] || t("settings.error_password_too_weak")}`,
      );
      setSaving(false);
      return;
    }
    ...
    try {
      ...
      const updatedConfig = {
        ...
        gist_encryption_password: settings.encryptionPassword,
        encryption_password: "", // 清空旧字段
      ...
      const createdGistID = await (
        window as any
      ).go.main.App.InitializeGistSync(settings.githubToken, settings.gistID);
      ...
      if (createdGistID && !settings.gistID) {
        console.log(
          "New Gist created, updating settings with ID:",
          createdGistID,
        );
      ...
      await (window as any).go.main.App.SetupGistEncryption(
        true,
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3 border-t pt-4 bg-amber-50 p-4 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-amber-900">
                  {t("settings.encryption_required")}
                </p>
            ...
            <div>
              <label className="block text-sm font-medium mb-2">
                {t("settings.encryption_password")}{" "}
                <span className="text-red-500">
                  {t("settings.encryption_password_required")}
                </span>
              </label>
              <input
                type="password"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      autoSyncInterval: parseInt(e.target.value) || 3600,
                    })
                  }
                  min="60"
                  className="w-full px-3 py-2 border border-input bg-background rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-ring"
          ...
          {message && (
            <div
              className={`p-3 rounded-md text-sm ${message.startsWith("Error") ? "bg-destructive/10 text-destructive" : "bg-green-100 text-green-700"}`}
            >
              {message}
            </div>
          )}
          ...
          <Button
            ...
            disabled={
              ...
              !settings.encryptionPassword ||
              !validatePasswordStrength(settings.encryptionPassword).isValid
            }
            className="w-full gap-2"
          >
            <Save className="w-4 h-4" />
            {saving
              ? t("settings.save_button_saving")
